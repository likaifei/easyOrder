...
function toUtf8Hex(codePoint){
	if (0x00 <= codePoint && codePoint <= 0x7f) return codePoint.toString(16)
	if (0x80 <= codePoint && codePoint <= 0x7ff) 
		return (0xC0 | (codePoint >> 6)).toString(16) + 
			(0x80 | (0x3F & codePoint)).toString(16)
	if ((0x800 <= codePoint && codePoint <= 0xd7ff) 
                  || (0xe000 <= codePoint && codePoint <= 0xffff)) {
		return URLEncode(String.fromCharCode(codePoint)).replace('%', '').replace('%', '').replace('+', '')
		return (codePoint >> 0xC | 0xE0).toString(16) + 
			(codePoint >> 0x6 & 0x3F | 0x80).toString(16) +
			(codePoint & 0x3F | 0x80).toString(16)
	}
}
function padHex(v){
	if(v.length % 2 != 0)
...
let str2hex = function(str){
　var val="";
	for(var i = 0; i < str.length; i++){
　　 val += padHex(toUtf8Hex(str.charCodeAt(i)));
	}
